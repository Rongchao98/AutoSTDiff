...
class IndividualEval(object):
    ...
    def get_gradius(self, trajs):
        ...
        # seq_len = 48
        for traj in trajs:
            xs = np.array([ck[1] for ck in traj])
            ys = np.array([ck[2] for ck in traj])
    ...
    def get_periodicity(self, trajs):
        ...
        for traj in trajs:
            ...
            for i in range(1, len(traj)):
                ...
                count += 1
            reps.append(count)
        reps = np.array(reps, dtype=float) / 7
        return reps
    ...
    def get_individual_jsds(self, t1, t2):
        ...
        :return:
        """
        # t1 = [[t2[0][0]] + t for t in t1]

        d1 = self.get_geodistances(t1)
        d2 = self.get_geodistances(t2)

        d1_dist, _ = EvalUtils.norm_arr_to_distribution(
            d1, 10000)
        d2_dist, _ = EvalUtils.norm_arr_to_distribution(
            d2, 10000)
        d_jsd = EvalUtils.get_js_divergence(d1_dist, d2_dist)
        

        g1 = self.get_gradius(t1) * 100
        g2 = self.get_gradius(t2) * 100
        g1_dist, _ = EvalUtils.norm_arr_to_distribution(
            g1, 10000)
        g2_dist, _ = EvalUtils.norm_arr_to_distribution(
            g2, 10000)
        ...
        du1_dist, _ = EvalUtils.norm_arr_to_distribution(
            du1, 336)
        du2_dist, _ = EvalUtils.norm_arr_to_distribution(
            du2, 336)
        du_jsd = EvalUtils.get_js_divergence(du1_dist, du2_dist)
        
        p1 = self.get_periodicity(t1)
        p2 = self.get_periodicity(t2)
        p1_dist, _ = EvalUtils.norm_arr_to_distribution(
            p1, 336)
        p2_dist, _ = EvalUtils.norm_arr_to_distribution(
            p2, 336)
...
def refine_traj(trajectories, time_limit):
    ...
    for trajectory in trajectories:
        if len(trajectory) > 1:
            ...
            # Filter out visits that exceed the time limit
            start_time = trajectory[0][0]
            filtered_trajectory = [visit for visit in trajectory if visit[0] - start_time <= time_limit]
...
def evaluate():
    evl = IndividualEval()
    gen_path = '01_01_24/TrajectoryResults/dataset_FoursquareNYC_timesteps_500/1400.pkl'
    # gen_path = 'TrajectoryResults/dataset_FoursquareNYC_timesteps_500/400.pkl'
    ori_path = 'dataset/FoursquareNYC/data_test.pkl'
    time_limit = 7 * 24 * 60 * 60

    f1 = open(gen_path, 'rb')
    f2 = open(ori_path, 'rb')
    gen_trajs = refine_traj(pickle.load(f1), time_limit)
    # gen_trajs = pickle.load(f1)
    ori_trajs = pickle.load(f2)

    print(evl.get_individual_jsds(ori_trajs, gen_trajs))
