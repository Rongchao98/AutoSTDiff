...
def get_memory_usage():
    return psutil.Process().memory_info().rss / (1024 * 1024 * 1024)  # Returns memory usage in MB
...
# x:(B*Seq_len, 1, 3), t:(B*Seq_len)
def extract(a, t, x_shape):
    b, *_ = t.shape # B*Seq_len
    out = a.gather(-1, t)
...
class GaussianDiffusion_ST(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.seq_length = seq_length  # 2+1 = 3
    ...
    # x:(B*Seq_len, 1, 3), t:(B*Seq_len), noise:(B*Seq_len, 1, 3)
    ...
    # x:(B*Seq_len, 1, 3), t:(B*Seq_len), cond:(B*Seq_len-1, 1, dim)
    def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False,cond=None):
        model_output = self.model(x, t, x_self_cond,cond=cond)  # (B*S, 1, 3)
    ...
    # single denoising process, x:(B*Seq_len, 1, 3)
    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond = None, clip_denoised = True, cond=None):  # (B*Seq_len, 1, 3)， t, (B*Seq_len-1, 1, dim)
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)  # (B*Seq_len)
        ...
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise  # (B*Seq_len, 1, 3)

        return pred_img, x_start, attn_weight

    # The entire denoising process
    @torch.no_grad()
    def p_sample_loop(self, shape, cond):  # (B*Seq_len, 1, 3), (B*Seq_len-1, 1, dim)
        batch, device = shape[0], self.betas.device

        img = torch.randn(shape, device=device)  # (B*Seq_len, 1, 3)

        x_start = None

        # for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):  # 500
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, cond=None):  # B*Seq_len, (B*Seq_len-1, 1, dim)
        seq_length, channels = self.seq_length, self.channels  # 3, 1
...
class ST_Diffusion(nn.Module):
    ...
    def get_attn(self, x, t, x_self_cond = None, cond = None):
        ...
        cond = self.cond_all(cond)  # (B*Seq_len-1, 1, 64)
        t_embedding = self.time_mlp(t).unsqueeze(dim=1)  # (B*Seq_len-1, 1, 64)


        cond_all = torch.cat((cond,t_embedding),dim=-1)  # (B*Seq_len-1, 1, 128)

        alpha_s = F.softmax(self.linear_s(cond_all), dim=-1).squeeze(dim=1)  # (B*Seq_len-1, 2)
        alpha_t = F.softmax(self.linear_t(cond_all), dim=-1).squeeze(dim=1)  # (B*Seq_len-1, 2)

        return alpha_s, alpha_t

    # Previous: x:(B*Seq_len, 1, 3), t:(B*Seq_len), cond:(B*Seq_len-1, 1, dim)
    # Current: x:(B*Seq_len, 1, D), t:(B*Seq_len), cond:(B*Seq_len-1, 1, dim)
    def forward(self, x, t, x_self_cond = None, cond = None):
        ...
        cond = self.cond_all(cond)  # (B, 1, u)
        t_embedding = self.time_mlp(t).unsqueeze(dim=1)  # (B, 1, u)


        cond_all = torch.cat((cond,t_embedding),dim=-1)

        alpha_s = F.softmax(self.linear_s(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2)  # (B, 2, 1)
        alpha_t = F.softmax(self.linear_t(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2)  # (B, 2, 1)
        ...
        x_spatial = self.linears_spatial[-1](x_spatial)  # (B, 1, u)
        x_temporal = self.linears_temporal[-1](x_temporal)

        x_output = torch.cat((x_temporal,x_spatial),dim=1) # (B, 2, u)

        x_output_t = (x_output * alpha_t).sum(dim=1,keepdim=True) # (B, 1, n)
        x_output_s = (x_output * alpha_s).sum(dim=1,keepdim=True)


        pred = torch.cat((self.output_temporal(x_output_t), self.output_spatial(x_output_s)), dim=-1)  # (B*Seq_len, 1, 3)
        return pred


class Model_all(nn.Module):
    def __init__(self, d2c, transformer, diffusion):
