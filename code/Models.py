...
class Encoder_ST(nn.Module):
    ...
    def forward(self, event_loc, event_time, non_pad_mask):
        ...
        slf_attn_mask_subseq = get_subsequent_mask(event_loc, dim=self.loc_dim)  # (B, S, S, 2), type=int
        slf_attn_mask_keypad = get_attn_key_pad_mask(seq_k=event_loc, seq_q=event_loc)  # (B, S, S, 2), type=boolean
        slf_attn_mask_keypad = slf_attn_mask_keypad.type_as(slf_attn_mask_subseq)

        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)  # (B, S, S, 2), type=boolean

        enc_output_temporal = self.temporal_enc(event_time, non_pad_mask)  # (B, S, dim(64))

        enc_output_loc = self.event_emb_loc(event_loc)  # (B, S, dim(64))

        enc_output = enc_output_temporal + enc_output_loc  # (B, S, dim(64))
        ...
        return enc_output, enc_output_temporal, enc_output_loc  # (B, S, dim(64))
...
class HEM(nn.Module):
    ...
    def __init__(
            self, d_model=64, d_rnn=256, d_inner=128,
            n_layers=4, n_head=4, d_k=16, d_v=16, dropout=0.1, device=None, loc_dim=2, CosSin=True):
        super().__init__()
    ...
    def forward(self, event_loc, event_time):
        ...
        enc_output, enc_output_temporal, enc_output_loc = self.encoder(event_loc, event_time, non_pad_mask)  # (B, S, dim)
