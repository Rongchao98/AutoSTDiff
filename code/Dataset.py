...
class EventData_1D(torch.utils.data.Dataset):
    ...
    def __init__(self, data):
        ...
        self.time = [[elem[0] for elem in inst] for inst in data]
        self.time_norm = [[elem[1] for elem in inst] for inst in data]
        self.loc_id = [[elem[2] for elem in inst] for inst in data]
    ...
    def __getitem__(self, idx):
        """ Each returned element is a list, which represents an event stream """
        return self.time[idx], self.time_norm[idx], self.loc_id[idx]
...
def pad_dis_loc(insts):
    ...
    return torch.tensor(batch_seq, dtype=torch.int64)
...
def collate_fn_1d(insts):
    ...
    time, time_norm, loc = list(zip(*insts))
    time = pad_time(time)
    time_norm = pad_time(time_norm)
    loc = pad_dis_loc(loc)
    return time, time_norm, loc


def get_dataloader(data, batch_size, D = 1, shuffle=True):
    """ Prepare dataloader. """
